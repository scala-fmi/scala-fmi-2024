...
import fmi.codecs.LibraryCodecs.unwrappedCodec
...
case class Book(id: BookId, name: String, authors: List[AuthorId], genre: String) derives Codec

case class AuthorId(id: String) extends AnyVal
object AuthorId:
  given Codec[AuthorId] = unwrappedCodec(AuthorId.apply)(_.id)

case class Author(id: AuthorId, name: String) derives Codec
