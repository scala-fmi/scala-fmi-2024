...
//    case FlatMap(io, f) =>
//      io match
//        case Pure(value) => f(value).unsafeRunStackSafe()
//        case Delay(delayedValue) => f(delayedValue()).unsafeRunStackSafe()
//        case FlatMap(ioPrev, fPrev) =>
//          ioPrev.flatMap(value => fPrev(value).flatMap(f)).unsafeRunStackSafe()
...
//      io match
//        case Pure(value) => f(value).unsafeRunStackSafe()
//        case Eval(delayedValue) => f(delayedValue()).unsafeRunStackSafe()
//        case FlatMap(ioPrev, fPrev) => ioPrev.flatMap(value => fPrev(value).flatMap(f)).unsafeRunStackSafe()
//
//        case Println(str) => println(str); f(()).unsafeRunStackSafe()
//        case Readln => f(StdIn.readLine()).unsafeRunStackSafe()
//    case Println(str) => println(str)
