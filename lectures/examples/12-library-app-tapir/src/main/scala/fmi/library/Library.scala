...
import fmi.library.LibraryCodecs.unwrappedCodec
...
object BookId:
  given Codec[BookId] = unwrappedCodec(BookId.apply)(_.id)
  given Schema[BookId] = Schema(SchemaType.SString())
  given tapir.Codec[String, BookId, CodecFormat.TextPlain] = tapir.Codec.string.map(BookId.apply)(_.id)
...
object AuthorId:
  given Codec[AuthorId] = unwrappedCodec(AuthorId.apply)(_.id)
  given Schema[AuthorId] = Schema(SchemaType.SString())
  given tapir.Codec[String, AuthorId, CodecFormat.TextPlain] = tapir.Codec.string.map(AuthorId.apply)(_.id)
