...
object MonoidDemo extends App {
  ...
  sum(List(1, 2, 3, 4, 5))(Monoid.intMultiplicativeMonoid) // 15

  {
    // An we can also explicitly change the context (with a new implicit val or with an import of an implicit val)

    implicit val ratMonoid: Monoid[Rational] = Rational.rationalMultiplicativeMonoid
    implicit val intMonoid: Monoid[Int] = Monoid.intMultiplicativeMonoid
